...
theory SeCaV_Appendix imports SeCaV begin

section \<open>Appendix: Completeness\<close>

subsection \<open>Miscellaneous Utilities\<close>

text \<open>Some facts about (in)finite sets\<close>

theorem set_inter_compl_diff [simp]: \<open>- A \<inter> B = B - A\<close> by (simp only: Diff_eq Int_commute)

subsection \<open>Terms and formulae\<close>
...
subsubsection \<open>Closed terms and formulae\<close>
...
primrec subst :: \<open>('a, 'b) form \<Rightarrow> 'a term \<Rightarrow> nat \<Rightarrow> ('a, 'b) form\<close>
  ...
  \<open>subst \<bottom> s k = \<bottom>\<close>
| \<open>subst \<top> s k = \<top>\<close>
...
theorem subst_closedt [simp]:
  ...
  shows \<open>closedt (Suc i) t \<Longrightarrow> closedt i (substt t u i)\<close>
    and \<open>closedts (Suc i) ts \<Longrightarrow> closedts i (substts ts u i)\<close>
  using u closedt_mono(1)
  by (induct t and ts rule: closedt.induct closedts.induct) auto

theorem subst_closed [simp]:
  \<open>closedt 0 t \<Longrightarrow> closed (Suc i) p \<Longrightarrow> closed i (subst p t i)\<close>
  by (induct p arbitrary: i t) simp_all

theorem subst_size_form [simp]: \<open>size_form (subst p t i) = size_form p\<close>
  by (induct p arbitrary: i t) simp_all

subsubsection \<open>Parameters\<close>
...
text \<open>
\label{sec:semantics}
In this subsection, we define evaluation functions for terms and formulae.
...
subsection \<open>Proof calculus\<close>
...
| ForallI: \<open>G \<turnstile> subst a (Fun n []) 0 \<Longrightarrow> list_all (\<lambda>p. n \<notin> params p) G \<Longrightarrow>
    n \<notin> params a \<Longrightarrow> G \<turnstile> Uni a\<close>
| ForallE: \<open>G \<turnstile> Uni a \<Longrightarrow> G \<turnstile> subst a t 0\<close>
| ExistsI: \<open>G \<turnstile> subst a t 0 \<Longrightarrow> G \<turnstile> Exi a\<close>
| ExistsE: \<open>G \<turnstile> Exi a \<Longrightarrow> subst a (Fun n []) 0 # G \<turnstile> b \<Longrightarrow>
...
proof -
  ...
  have \<open>[Exi ?forall] \<turnstile> Exi ?forall\<close>
    by (simp add: Assum)
  moreover { have \<open>[subst ?forall (Fun 1 []) 0, Exi ?forall] \<turnstile> Uni (Pre p [Fun 1 [], Var 0])\<close>
      by (simp add: Assum)
    moreover have \<open>[subst (Pre p [Fun 1 [], Var 0]) (Fun 0 []) 0, subst ?forall (Fun 1 []) 0,
      Exi ?forall] \<turnstile> subst (Pre p [Var 0, Fun 0 []]) (Fun 1 []) 0\<close>
      by (simp add: Assum)
    ultimately have \<open>[subst ?forall (Fun 1 []) 0, Exi ?forall] \<turnstile>
      subst (Pre p [Var 0, Fun 0 []]) (Fun 1 []) 0\<close>
      by (rule ForallE') }
  then have \<open>[subst ?forall (Fun 1 []) 0, Exi ?forall] \<turnstile> Exi (Pre p [Var 0, Fun 0 []])\<close>
    by (rule ExistsI)
  ...
  ultimately have \<open>[Exi ?forall] \<turnstile> Exi (Pre p [Var 0, Fun 0 []])\<close>
    by (rule ExistsE)
  then have \<open>[Exi ?forall] \<turnstile> subst (Exi (Pre p [Var 0, Var 1])) (Fun 0 []) 0\<close>
    by simp
...
proof -
  ...
  then have \<open>Pre P [Fun 0 []] # ?G \<turnstile> Uni (Pre P [Var 0])\<close>
    by (rule FFE)
  then have \<open>?G \<turnstile> subst ?impl (Fun 0 []) 0\<close>
    using ImplI by simp
  then have \<open>?G \<turnstile> Exi ?impl\<close>
    by (rule ExistsI)
  ultimately have \<open>?G \<turnstile> \<bottom>\<close>
    by (rule NegE)
  then have \<open>?G' \<turnstile> subst (Pre P [Var 0]) (Fun 0 []) 0\<close>
    using Class by simp
  ...
  ultimately have \<open>?G' \<turnstile> Uni (Pre P [Var 0])\<close>
    by (rule ForallI)
  then have \<open>[Neg (Exi ?impl)] \<turnstile> subst ?impl (Var 0) 0\<close>
    using ImplI by simp
...
next
  ...
  proof (intro allI)
    ...
    have \<open>\<forall>z. e, (f(n := \<lambda>x. z)), g, G \<Turnstile> (subst a (Fun n []) 0)\<close>
...
text \<open>
The goal of this subsection is to prove completeness of the natural deduction
...
subsubsection \<open>Consistent sets\<close>

text \<open>
\label{sec:consistent-sets}
In this subsection, we describe an abstract criterion for consistent sets.
...
definition consistency :: \<open>('a, 'b) form set set \<Rightarrow> bool\<close> where
  \<open>consistency C = (\<forall>S. S \<in> C \<longrightarrow>
     ...
     (\<forall>P t. closedt 0 t \<longrightarrow> Uni P \<in> S \<longrightarrow> S \<union> {subst P t 0} \<in> C) \<and>
     (\<forall>P t. closedt 0 t \<longrightarrow> Neg (Exi P) \<in> S \<longrightarrow> S \<union> {Neg (subst P t 0)} \<in> C) \<and>
     (\<forall>P. Exi P \<in> S \<longrightarrow> (\<exists>x. S \<union> {subst P (Fun x []) 0} \<in> C)) \<and>
     (\<forall>P. Neg (Uni P) \<in> S \<longrightarrow> (\<exists>x. S \<union> {Neg (subst P (Fun x []) 0)} \<in> C)))\<close>
...
proof (intro allI impI conjI)
  ...
  { fix P and t :: \<open>'a term\<close>
    ...
    then have \<open>psubst f (Uni P) \<in> ?S'\<close>
      by blast
    then have \<open>?S' \<union> {subst (psubst f P) (psubstt f t) 0} \<in> C\<close>
      using \<open>closedt 0 t\<close> conc sc by (simp add: consistency_def)
    then show \<open>S \<union> {subst P t 0} \<in> ?C'\<close>
      unfolding mk_alt_consistency_def by auto }
  ...
  { fix P and t :: \<open>'a term\<close>
    ...
    then have \<open>psubst f (Neg (Exi P)) \<in> ?S'\<close>
      by blast
    then have \<open>?S' \<union> {Neg (subst (psubst f P) (psubstt f t) 0)} \<in> C\<close>
      using \<open>closedt 0 t\<close> conc sc by (simp add: consistency_def)
    then show \<open>S \<union> {Neg (subst P t 0)} \<in> ?C'\<close>
      unfolding mk_alt_consistency_def by auto }
  ...
  { fix P :: \<open>('a, 'b) form\<close> and x f'
    ...
    then have \<open>\<exists>y. ?S' \<union> {subst (psubst f P) (Fun y []) 0} \<in> C\<close>
      using conc sc by (simp add: consistency_def)
    then obtain y where \<open>?S' \<union> {subst (psubst f P) (Fun y []) 0} \<in> C\<close>
      by blast

    moreover have \<open>psubst (f(x := y)) ` S = ?S'\<close>
      using calculation by (simp cong add: image_cong)
    moreover have \<open>psubst (f(x := y)) `
        S \<union> {subst (psubst (f(x := y)) P) (Fun ((f(x := y)) x) []) 0} \<in> C\<close>
      using calculation by auto
    ultimately have \<open>\<exists>f. psubst f ` S \<union> {subst (psubst f P) (Fun (f x) []) 0} \<in> C\<close>
      by blast
    then show \<open>S \<union> {subst P (Fun x []) 0} \<in> ?C'\<close>
  ...
  { fix P :: \<open>('a, 'b) form\<close> and x
    ...
    moreover have \<open>psubst f (Neg (Uni P)) \<in> ?S'\<close>
      using calculation by blast
    then have \<open>\<exists>y. ?S' \<union> {Neg (subst (psubst f P) (Fun y []) 0)} \<in> C\<close>
      using conc sc by (simp add: consistency_def)
    then obtain y where \<open>?S' \<union> {Neg (subst (psubst f P) (Fun y []) 0)} \<in> C\<close>
      by blast

    moreover have \<open>psubst (f(x := y)) ` S = ?S'\<close>
      using calculation by (simp cong add: image_cong)
    moreover have \<open>psubst (f(x := y)) `
    S \<union> {Neg (subst (psubst (f(x := y)) P) (Fun ((f(x := y)) x) []) 0)} \<in> C\<close>
      using calculation by auto
    ultimately have \<open>\<exists>f. psubst f ` S \<union> {Neg (subst (psubst f P) (Fun (f x) []) 0)} \<in> C\<close>
      by blast
    then show \<open>S \<union> {Neg (subst P (Fun x []) 0)} \<in> ?C'\<close>
      unfolding mk_alt_consistency_def by simp }
qed
...
subsubsection \<open>Closure under subsets\<close>
...
proof (intro allI impI conjI)
  ...
  { fix P and t :: \<open>'a term\<close>
    ...
    then have \<open>Uni P \<in> x\<close>
      using \<open>S \<subseteq> x\<close> by blast
    then have \<open>x \<union> {subst P t 0} \<in> C\<close>
      using \<open>closedt 0 t\<close> \<open>x \<in> C\<close> conc unfolding consistency_def by blast
    then show \<open>S \<union> {subst P t 0} \<in> close C\<close>
      using \<open>S \<subseteq> x\<close> subset_in_close by blast }
  ...
  { fix P and t :: \<open>'a term\<close>
    ...
    then have \<open>Neg (Exi P) \<in> x\<close>
      using \<open>S \<subseteq> x\<close> by blast
    then have \<open>x \<union> {Neg (subst P t 0)} \<in> C\<close>
      using \<open>closedt 0 t\<close> \<open>x \<in> C\<close> conc unfolding consistency_def by blast
    then show \<open>S \<union> {Neg (subst P t 0)} \<in> close C\<close>
      using \<open>S \<subseteq> x\<close> subset_in_close by blast }
  ...
  { fix P
    ...
    then have \<open>Exi P \<in> x\<close>
      using \<open>S \<subseteq> x\<close> by blast
    then have \<open>\<exists>c. x \<union> {subst P (Fun c []) 0} \<in> C\<close>
      using \<open>x \<in> C\<close> conc unfolding consistency_def by blast
    then show \<open>\<exists>c. S \<union> {subst P (Fun c []) 0} \<in> close C\<close>
      using \<open>S \<subseteq> x\<close> subset_in_close by blast }
  ...
  { fix P
    ...
    then have \<open>Neg (Uni P) \<in> x\<close>
      using \<open>S \<subseteq> x\<close> by blast
    then have \<open>\<exists>c. x \<union> {Neg (subst P (Fun c []) 0)} \<in> C\<close>
      using \<open>x \<in> C\<close> conc unfolding consistency_def by simp
    then show \<open>\<exists>c. S \<union> {Neg (subst P (Fun c []) 0)} \<in> close C\<close>
      using \<open>S \<subseteq> x\<close> subset_in_close by blast }
qed
...
subsubsection \<open>Finite character\<close>

text \<open>
\label{sec:finiteness}
In this subsection, we show that an alternative consistency property can
...
proof (intro allI impI conjI)
  ...
  { fix P and t :: \<open>'a term\<close>
    ...
    show \<open>S \<union> {subst P t 0} \<in> mk_finite_char C\<close>
      unfolding mk_finite_char_def
    proof (intro allI impI CollectI)
      fix S'
      let ?S' = \<open>S' - {subst P t 0} \<union> {Uni P}\<close>

      assume \<open>S' \<subseteq> S \<union> {subst P t 0}\<close> and \<open>finite S'\<close>
      then have \<open>?S' \<subseteq> S\<close>
        using * by blast
      moreover have \<open>finite ?S'\<close>
        using \<open>finite S'\<close> by blast
      ultimately have \<open>?S' \<in> C\<close>
        using finc by blast
      then have \<open>?S' \<union> {subst P t 0} \<in> C\<close>
  ...
  { fix P and t :: \<open>'a term\<close>
    ...
    show \<open>S \<union> {Neg (subst P t 0)} \<in> mk_finite_char C\<close>
      unfolding mk_finite_char_def
    proof (intro allI impI CollectI)
      fix S'
      let ?S' = \<open>S' - {Neg (subst P t 0)} \<union> {Neg (Exi P)}\<close>

      assume \<open>S' \<subseteq> S \<union> {Neg (subst P t 0)}\<close> and \<open>finite S'\<close>
      then have \<open>?S' \<subseteq> S\<close>
        using * by blast
      moreover have \<open>finite ?S'\<close>
        using \<open>finite S'\<close> by blast
      ultimately have \<open>?S' \<in> C\<close>
        using finc by blast
      then have \<open>?S' \<union> {Neg (subst P t 0)} \<in> C\<close>
        using altconc \<open>closedt 0 t\<close> unfolding alt_consistency_def by simp
      then show \<open>S' \<in> C\<close>
        using sc by blast
    qed }
  ...
  { fix P x
    ...
    show \<open>S \<union> {subst P (Fun x []) 0} \<in> mk_finite_char C\<close>
      unfolding mk_finite_char_def
    proof (intro allI impI CollectI)
      fix S'
      let ?S' = \<open>S' - {subst P (Fun x []) 0} \<union> {Exi P}\<close>

      assume \<open>S' \<subseteq> S \<union> {subst P (Fun x []) 0}\<close> and \<open>finite S'\<close>
      then have \<open>?S' \<subseteq> S\<close>
        using * by blast
      moreover have \<open>finite ?S'\<close>
        using \<open>finite S'\<close> by blast
      ...
      ultimately have \<open>?S' \<union> {subst P (Fun x []) 0} \<in> C\<close>
  ...
  { fix P x
    ...
    show \<open>S \<union> {Neg (subst P (Fun x []) 0)} \<in> mk_finite_char C\<close>
      unfolding mk_finite_char_def
    proof (intro allI impI CollectI)
      fix S'
      let ?S' = \<open>S' - {Neg (subst P (Fun x []) 0)} \<union> {Neg (Uni P)}\<close>

      assume \<open>S' \<subseteq> S \<union> {Neg (subst P (Fun x []) 0)}\<close> and \<open>finite S'\<close>
      then have \<open>?S' \<subseteq> S\<close>
        using * by blast
      moreover have \<open>finite ?S'\<close>
        using \<open>finite S'\<close> by blast
      ...
      moreover have \<open>\<forall>a \<in> ?S'. x \<notin> params a\<close>
        using \<open>\<forall>a \<in> S. x \<notin> params a\<close> \<open>?S' \<subseteq> S\<close> by blast
      ultimately have \<open>?S' \<union> {Neg (subst P (Fun x []) 0)} \<in> C\<close>
...
subsubsection \<open>Enumerating datatypes\<close>

text \<open>
\label{sec:enumeration}
In the following subsection, we will show that elements of datatypes
...
paragraph \<open>Enumerating pairs of natural numbers\<close>
...
paragraph \<open>Enumerating trees\<close>
...
paragraph \<open>Enumerating lists\<close>
...
| \<open>list_of_btree a (Branch (Branch vb vc) va) = undefined\<close>
...
paragraph \<open>Enumerating terms\<close>
...
| \<open>term_of_btree a (Branch (Branch vb vc) va) = undefined\<close>
...
| \<open>form_of_btree a b (Leaf (Suc (Suc v))) = undefined\<close>
| \<open>form_of_btree a b (Branch (Leaf (Suc (Suc (Suc (Suc (Suc (Suc (Suc vb)))))))) va) = undefined\<close>
| \<open>form_of_btree a b (Branch (Leaf (Suc (Suc (Suc 0)))) (Leaf v)) = undefined\<close>
| \<open>form_of_btree a b (Branch (Leaf (Suc (Suc 0))) (Leaf vb)) = undefined\<close>
| \<open>form_of_btree a b (Branch (Leaf (Suc 0)) (Leaf v)) = undefined\<close>
| \<open>form_of_btree a b (Branch (Branch vb vc) va) = undefined\<close>
| \<open>form_of_btree a b (Branch (Leaf 0) (Leaf vb)) = undefined\<close>
| \<open>form_of_btree a b (Branch (Leaf 0) (Branch (Branch vd ve) vc)) = undefined\<close>
| \<open>form_of_btree a b (Branch (Leaf 0) (Branch (Leaf v) (Branch vd ve))) = undefined\<close>
...
text \<open>
As an alternative to the enumerations in this subsection, one can delete the above abbreviations and
...
subsubsection \<open>Extension to maximal consistent sets\<close>
...
proof -
  ...
  moreover have \<open>Exi p \<in> ?S'\<close>
    using * by simp
  then have \<open>\<forall>x. x \<notin> (\<Union>p \<in> ?S'. params p) \<longrightarrow> ?S' \<union> {subst p (Fun x []) 0} \<in> C\<close>
    using \<open>?S' \<in> C\<close> \<open>alt_consistency C\<close>
    unfolding alt_consistency_def by simp
  ultimately have \<open>(?S' \<union> {subst p (Fun (SOME k. k \<notin> (\<Union>p \<in> ?S'. params p)) []) 0}) \<in> C\<close>
...
proof -
  ...
  moreover have \<open>Neg (Uni p) \<in> ?S'\<close>
    using * by simp
  then have \<open>\<forall>x. x \<notin> (\<Union>p \<in> ?S'. params p) \<longrightarrow> ?S' \<union> {Neg (subst p (Fun x []) 0)} \<in> C\<close>
    using \<open>?S' \<in> C\<close> \<open>alt_consistency C\<close>
    unfolding alt_consistency_def by simp
  ultimately have \<open>(?S' \<union> {Neg (subst p (Fun (SOME k. k \<notin> (\<Union>p \<in> ?S'. params p)) []) 0)}) \<in> C\<close>
    by (metis (mono_tags, lifting) someI2)
  then show ?thesis
    using assms * by simp
qed
...
subsubsection \<open>Hintikka sets and Herbrand models\<close>
...
next
  ...
  next
    ...
    proof (intro conjI impI)
      ...
      proof (rule allI)
        ...
        then have *: \<open>\<forall>P t. closedt 0 t \<longrightarrow> Uni P \<in> H \<longrightarrow> subst P t 0 \<in> H\<close>
          using hin unfolding hintikka_def by blast
        from \<open>closed 0 (Uni P)\<close>
        have \<open>closed (Suc 0) P\<close> by simp

        have \<open>(subst P (term_of_hterm z) 0, Uni P) \<in> measure size_form \<longrightarrow>
              subst P (term_of_hterm z) 0 \<in> H \<longrightarrow> closed 0 (subst P (term_of_hterm z) 0) \<longrightarrow>
              ?eval (subst P (term_of_hterm z) 0)\<close>
    ...
    next
      ...
      then have \<open>Neg (Uni P) \<in> H\<close>
        using Uni by simp
      then have \<open>\<exists>t. closedt 0 t \<and> Neg (subst P t 0) \<in> H\<close>
        using Uni hin unfolding hintikka_def by blast
      then obtain t where *: \<open>closedt 0 t \<and> Neg (subst P t 0) \<in> H\<close>
        by blast
      then have \<open>closed 0 (subst P t 0)\<close>
      ...
      have \<open>(subst P t 0, Uni P) \<in> measure size_form \<longrightarrow>
        ...
        using Uni wf by blast
      then have \<open>?eval (Neg (subst P t 0))\<close>
        using Uni * \<open>closed 0 (subst P t 0)\<close> by simp
  ...
  next
    ...
    proof (intro conjI impI allI)
      assume \<open>x \<in> H\<close> and \<open>closed 0 x\<close>
      then have \<open>\<exists>t. closedt 0 t \<and> subst P t 0 \<in> H\<close>
        using Exi hin unfolding hintikka_def by blast
      then obtain t where *: \<open>closedt 0 t \<and> subst P t 0 \<in> H\<close>
        by blast
      then have \<open>closed 0 (subst P t 0)\<close>
      ...
      then have \<open>?eval (subst P t 0)\<close>
        using Exi * \<open>closed 0 (subst P t 0)\<close> by simp
    ...
    next
      ...
      proof (rule allI)
        ...
        then have *: \<open>\<forall>P t. closedt 0 t \<longrightarrow> Neg (Exi P) \<in> H \<longrightarrow> Neg (subst P t 0) \<in> H\<close>
          using hin unfolding hintikka_def by blast
        from \<open>closed 0 (Neg (Exi P))\<close>
        have \<open>closed (Suc 0) P\<close> by simp

        have \<open>(subst P (term_of_hterm z) 0, Exi P) \<in> measure size_form \<longrightarrow>
              (Neg (subst P (term_of_hterm z) 0) \<in> H \<longrightarrow> closed 0 (subst P (term_of_hterm z) 0) \<longrightarrow>
              ?eval (Neg (subst P (term_of_hterm z) 0)))\<close>
          using Exi wf by blast
...
lemma Exists_in_extend:
  ...
  shows \<open>subst P (Fun (SOME k. k \<notin> (\<Union>p \<in> extend S C f n \<union> {f n}. params p)) []) 0 \<in>
...
proof -
  ...
  then have \<open>extend S C f (Suc n) = (?S' \<union> {subst (dest_Exists (f n)) ?t 0})\<close>
    using \<open>?S' \<in> C\<close> by simp
  also have \<open>\<dots> = (?S' \<union> {subst (dest_Exists (Exi P)) ?t 0})\<close>
    using \<open>Exi P = f n\<close> by simp
  also have \<open>\<dots> = (?S' \<union> {subst P ?t 0})\<close>
    by simp
  finally show ?thesis
    by blast
qed
...
lemma Neg_Forall_in_extend:
  ...
  shows \<open>Neg (subst P (Fun (SOME k. k \<notin> (\<Union>p \<in> extend S C f n \<union> {f n}. params p)) []) 0) \<in>
          extend S C f (Suc n)\<close>
...
proof -
  ...
  then have \<open>extend S C f (Suc n) = (?S' \<union> {Neg (subst (dest_Forall (dest_Neg (f n))) ?t 0)})\<close>
    using \<open>?S' \<in> C\<close> \<open>f n \<noteq> Exi P\<close> by auto
  also have \<open>\<dots> = (?S' \<union> {Neg (subst (dest_Forall (dest_Neg (Neg (Uni P)))) ?t 0)})\<close>
    using \<open>Neg (Uni P) = f n\<close> by simp
  also have \<open>\<dots> = (?S' \<union> {Neg (subst P ?t 0)})\<close>
    by simp
  finally show ?thesis
    by blast
qed
...
proof (intro allI impI conjI)
  ...
  { fix P and t :: \<open>nat term\<close>
    ...
    then have \<open>?H \<union> {subst P t 0} \<in> C\<close>
      using \<open>?H \<in> C\<close> altc unfolding alt_consistency_def by blast
    then show \<open>subst P t 0 \<in> ?H\<close>
      using \<open>maximal ?H C\<close> unfolding maximal_def by fast }

  { fix P and t :: \<open>nat term\<close>
    assume \<open>Neg (Exi P) \<in> ?H\<close> and \<open>closedt 0 t\<close>
    then have \<open>?H \<union> {Neg (subst P t 0)} \<in> C\<close>
      using \<open>?H \<in> C\<close> altc unfolding alt_consistency_def by blast
    then show \<open>Neg (subst P t 0) \<in> ?H\<close>
      using \<open>maximal ?H C\<close> unfolding maximal_def by fast }
  ...
  { fix P
    ...
    then have \<open>subst P ?t 0 \<in> extend S C f (Suc n)\<close>
      using * Exists_in_extend by blast
    then have \<open>subst P ?t 0 \<in> ?H\<close>
      using Extend_def by blast
    then show \<open>\<exists>t. closedt 0 t \<and> subst P t 0 \<in> ?H\<close>
  ...
  { fix P
    ...
    then have \<open>extend S C f n \<union> {f n} \<in> C\<close>
      using Extend_def \<open>Extend S C f \<in> C\<close> fin_ch finite_char_closed
      unfolding subset_closed_def by metis
    then have \<open>Neg (subst P ?t 0) \<in> extend S C f (Suc n)\<close>
      using * Neg_Forall_in_extend by blast
    then have \<open>Neg (subst P ?t 0) \<in> ?H\<close>
      using Extend_def by blast
    then show \<open>\<exists>t. closedt 0 t \<and> Neg (subst P t 0) \<in> ?H\<close>
      using \<open>closedt 0 ?t\<close> by blast }
qed

subsubsection \<open>Model existence theorem\<close>
...
subsubsection \<open>Completeness for Natural Deduction\<close>
...
proof (intro conjI allI impI notI)
  ...
  { fix P and t :: \<open>'a term\<close>
    ...
    then have \<open>G \<turnstile> Uni P\<close>
      using Assum * by blast
    then have \<open>G \<turnstile> subst P t 0\<close>
      using ForallE by blast

    { assume \<open>subst P t 0 # G \<turnstile> \<bottom>\<close>
      then have \<open>G \<turnstile> \<bottom>\<close>
        using cut \<open>G \<turnstile> subst P t 0\<close> by blast
      then have False
        using \<open>\<not> G \<turnstile> \<bottom>\<close> by blast }
    then have \<open>\<not> subst P t 0 # G \<turnstile> \<bottom>\<close>
      by blast
  ...
  { fix P and t :: \<open>'a term\<close>
    ...
    then have \<open>G \<turnstile> Neg (Exi P)\<close>
      using Assum * by blast
    then have \<open>subst P t 0 \<in> set (subst P t 0 # G)\<close>
      by (simp add: Assum)
    then have \<open>subst P t 0 # G \<turnstile> subst P t 0\<close>
      using Assum by blast
    then have \<open>subst P t 0 # G \<turnstile> Exi P\<close>
      using ExistsI by blast
    moreover have \<open>subst P t 0 # G \<turnstile> Neg (Exi P)\<close>
      using * \<open>Neg (Exi P) \<in> S\<close> by (simp add: Assum)
    ultimately have \<open>subst P t 0 # G \<turnstile> \<bottom>\<close>
      using NegE by blast
    then have \<open>G \<turnstile> Neg (subst P t 0)\<close>
      using NegI by blast

    { assume \<open>Neg (subst P t 0) # G \<turnstile> \<bottom>\<close>
      then have \<open>G \<turnstile> \<bottom>\<close>
        using cut \<open>G \<turnstile> Neg (subst P t 0)\<close> by blast
      then have False
        using \<open>\<not> G \<turnstile> \<bottom>\<close> by blast }
    then have \<open>\<not> Neg (subst P t 0) # G \<turnstile> \<bottom>\<close>
      by blast
  ...
  { fix P
    ...
    { assume \<open>subst P (Fun x []) 0 # G \<turnstile> \<bottom>\<close>
      ...
      then have False
        using \<open>\<not> G \<turnstile> \<bottom>\<close>
        by blast}
    then have \<open>\<not> subst P (Fun x []) 0 # G \<turnstile> \<bottom>\<close>
      by blast
  ...
  { fix P
    ...
    have \<open>Neg (subst P (Fun x []) 0) # ?x # G \<turnstile> (subst (Neg P) (Fun x []) 0)\<close>
      by (simp add: Assum)
    then have \<open>Neg (subst P (Fun x []) 0) # ?x # G \<turnstile> Exi (Neg P)\<close>
      using ExistsI by blast
    moreover have \<open>Neg (subst P (Fun x []) 0) # ?x # G \<turnstile> ?x\<close>
      by (simp add: Assum)
    ultimately have \<open>Neg (subst P (Fun x []) 0) # ?x # G \<turnstile> \<bottom>\<close>
      using NegE by blast
    then have \<open>?x # G \<turnstile> subst P (Fun x []) 0\<close>
      using Class by blast
    ...
    { assume \<open>Neg (subst P (Fun x []) 0) # G \<turnstile> \<bottom>\<close>
      moreover have \<open>list_all (\<lambda>p. x \<notin> params p) G\<close>
        using ** by (simp add: list_all_iff)
      moreover have \<open>x \<notin> params P\<close>
        using ** by simp
      ...
      then have False
        using \<open>\<not> G \<turnstile> \<bottom>\<close>
        by blast}
    then have \<open>\<not> Neg (subst P (Fun x []) 0) # G \<turnstile> \<bottom>\<close>
      by blast
...
subsection \<open>L\"owenheim-Skolem theorem\<close>
...
theorem sat_consistency:
  \<open>consistency
    {S. infinite (- (\<Union>p \<in> S. params p)) \<and> (\<exists>f. \<forall>(p::('a, 'b)form) \<in> S. semantics e f g p)}\<close>
  unfolding consistency_def
proof (intro allI impI conjI)
  ...
  { fix P and t :: \<open>'a term\<close>
    ...
    then have \<open>\<forall>x \<in> S \<union> {Uni P}. semantics e f g x\<close>
      using * by blast
    then have \<open>\<forall>x \<in> S \<union> {subst P t 0}. semantics e f g x\<close>
      by simp
    moreover have \<open>infinite (- (\<Union>p \<in> S \<union> {subst P t 0}. params p))\<close>
      using inf_params by simp
    ultimately show \<open>S \<union> {subst P t 0} \<in> ?C\<close>
      by blast }
  ...
  { fix P and t :: \<open>'a term\<close>
    ...
    then have \<open>\<forall>x \<in> S \<union> {Neg (Exi P)}. semantics e f g x\<close>
      using * by blast
    then have \<open>\<forall>x \<in> S \<union> {Neg (subst P t 0)}. semantics e f g x\<close>
      by simp
    moreover have \<open>infinite (- (\<Union>p \<in> S \<union> {Neg (subst P t 0)}. params p))\<close>
      using inf_params by simp
    ultimately show \<open>S \<union> {Neg (subst P t 0)} \<in> ?C\<close>
      by blast }
  ...
  { fix P
    ...
    moreover have \<open>\<forall>p \<in> S. semantics e (f(x := \<lambda>y. z)) g p\<close>
      using * ** by simp
    moreover have \<open>infinite (- (\<Union>p \<in> S \<union> {subst P (Fun x []) 0}. params p))\<close>
      using inf_params by simp
    ultimately have \<open>S \<union> {subst P (Fun x []) 0} \<in> {S. infinite (- (\<Union>p \<in> S. params p)) \<and>
                      (\<forall>p \<in> S. semantics e (f(x := \<lambda>y. z)) g p)}\<close>
      by simp
    then show \<open>\<exists>x. S \<union> {subst P (Fun x []) 0} \<in> ?C\<close>
      by blast }
  ...
  { fix P
    ...
    moreover have \<open>infinite (- (\<Union>p \<in> S \<union> {subst P (Fun x []) 0}. params p))\<close>
      using inf_params by simp
    ultimately have \<open>S \<union> {Neg (subst P (Fun x []) 0)} \<in> {S. infinite (- (\<Union>p \<in> S. params p)) \<and>
                      (\<forall>p \<in> S. semantics e (f(x := \<lambda>y. z)) g p)}\<close>
      by simp
    then show \<open>\<exists>x. S \<union> {Neg (subst P (Fun x []) 0)} \<in> ?C\<close>
      by blast }
qed
...
subsection \<open>Completeness for open formulas\<close>
...
subsubsection \<open>Substitution for constants\<close>
...
lemma psubst_new_away [simp]:
  \<open>new' fresh p \<Longrightarrow> psubst (id(fresh := c)) (psubst (id(c := fresh)) p) = p\<close>
  by (induct p) simp_all
...
subsubsection \<open>Weakening assumptions\<close>
...
subsubsection \<open>Implications and assumptions\<close>
...
subsubsection \<open>Closure elimination\<close>
...
subsubsection \<open>Completeness for Closed Formulas\<close>
...
subsubsection \<open>Open Formulas\<close>
...
proof -
  ...
  moreover have \<open>\<forall>(e :: nat \<Rightarrow> nat hterm) f g. list_all (semantics e f g) (map (sub_consts cs) G)
    \<longrightarrow> semantics e f g (sub_consts cs p)\<close>
...
subsubsection \<open>Tableau Calculus Equivalence\<close>
...
proof -
  have \<open>\<stileturn> Neg p # ps\<close>
    using assms FOL_Tableau.tableau_completeness unfolding FOL_Tableau.tableauproof_def by simp
  then show ?thesis
...
subsection \<open>SeCaV: Sound & Complete\<close>

primrec from_tm and from_tm_list where
  \<open>from_tm (SeCaV.Var n) = Var n\<close> |
  \<open>from_tm (SeCaV.Fun a ts) = Fun a (from_tm_list ts)\<close> |
  \<open>from_tm_list [] = []\<close> |
  \<open>from_tm_list (t # ts) = from_tm t # from_tm_list ts\<close>

primrec from_fm where
  \<open>from_fm (SeCaV.Pre b ts) = Pre b (from_tm_list ts)\<close> |
  \<open>from_fm (SeCaV.Con p q) = Con (from_fm p) (from_fm q)\<close> |
  \<open>from_fm (SeCaV.Dis p q) = Dis (from_fm p) (from_fm q)\<close> |
  \<open>from_fm (SeCaV.Imp p q) = Imp (from_fm p) (from_fm q)\<close> |
  \<open>from_fm (SeCaV.Neg p) = Neg (from_fm p)\<close> |
  \<open>from_fm (SeCaV.Uni p) = Uni (from_fm p)\<close> |
  \<open>from_fm (SeCaV.Exi p) = Exi (from_fm p)\<close>

primrec to_tm and to_tm_list where
  \<open>to_tm (Var n) = SeCaV.Var n\<close> |
  \<open>to_tm (Fun a ts) = SeCaV.Fun a (to_tm_list ts)\<close> |
  \<open>to_tm_list [] = []\<close> |
  \<open>to_tm_list (t # ts) = to_tm t # to_tm_list ts\<close>

primrec to_fm where
  \<open>to_fm \<bottom> = SeCaV.Neg (SeCaV.Imp (SeCaV.Pre 0 []) (SeCaV.Pre 0 []))\<close> |
  \<open>to_fm \<top> = SeCaV.Imp (SeCaV.Pre 0 []) (SeCaV.Pre 0 [])\<close> |
  \<open>to_fm (Pre b ts) = SeCaV.Pre b (to_tm_list ts)\<close> |
  \<open>to_fm (Con p q) = SeCaV.Con (to_fm p) (to_fm q)\<close> |
  \<open>to_fm (Dis p q) = SeCaV.Dis (to_fm p) (to_fm q)\<close> |
  \<open>to_fm (Imp p q) = SeCaV.Imp (to_fm p) (to_fm q)\<close> |
  \<open>to_fm (Neg p) = SeCaV.Neg (to_fm p)\<close> |
  \<open>to_fm (Uni p) = SeCaV.Uni (to_fm p)\<close> |
  \<open>to_fm (Exi p) = SeCaV.Exi (to_fm p)\<close>
...
lemma Truth [simp]: \<open>\<tturnstile> SeCaV.Imp (SeCaV.Pre 0 []) (SeCaV.Pre 0 []) # z\<close>
  using SeCaV.AlphaImp SeCaV.Basic SeCaV.Ext ext.simps member.simps(2) by metis

lemma paramst [simp]:
  \<open>new_term' c t = SeCaV.new_term c (to_tm t)\<close>
  \<open>new_list' c l = SeCaV.new_list c (to_tm_list l)\<close>
  by (induct t and l rule: paramst.induct paramsts.induct) simp_all

lemma params [iff]: \<open>new' c p \<longleftrightarrow> SeCaV.new c (to_fm p)\<close>
  by (induct p) simp_all

lemma list_params [iff]: \<open>news' c z \<longleftrightarrow> SeCaV.news c (map to_fm z)\<close>
  by (induct z) simp_all

lemma liftt [simp]:
  \<open>to_tm (liftt t) = inc_term (to_tm t)\<close>
  \<open>to_tm_list (liftts l) = inc_list (to_tm_list l)\<close>
  by (induct t and l rule: liftt.induct liftts.induct) simp_all

lemma substt [simp]:
  \<open>to_tm (substt t s v) = sub_term v (to_tm s) (to_tm t)\<close>
  \<open>to_tm_list (substts l s v) = sub_list v (to_tm s) (to_tm_list l)\<close>
  by (induct t and l rule: substt.induct substts.induct) simp_all

lemma subst [simp]: \<open>to_fm (subst A t s) = sub s (to_tm t) (to_fm A)\<close>
...
lemma evalt [simp]:
  \<open>SeCaV.semantics_term e f t = semantics_term e f (from_tm t)\<close>
  \<open>SeCaV.semantics_list e f ts = semantics_list e f (from_tm_list ts)\<close>
  by (induct t and ts rule: from_tm.induct from_tm_list.induct) simp_all

lemma shift [simp]: \<open>SeCaV.shift e 0 x = e\<langle>0:x\<rangle>\<close>
  unfolding SeCaV.shift_def shift_def by simp

lemma semantics [iff]: \<open>SeCaV.semantics e f g p \<longleftrightarrow> semantics e f g (from_fm p)\<close>
  by (induct p arbitrary: e) simp_all

abbreviation valid' ("\<then> _" 0) where
  \<open>(\<then> p) \<equiv> \<forall>(e :: _ \<Rightarrow> nat hterm) f g. SeCaV.semantics e f g p\<close>

theorem complete_sound: \<open>\<then> p \<Longrightarrow> \<tturnstile> [p]\<close> \<open>\<tturnstile> [q] \<Longrightarrow> SeCaV.semantics e f g q\<close>
  by (metis to_from_fm sim semantics list.map FOL_Sequent.SC_completeness) (use sound in force)
